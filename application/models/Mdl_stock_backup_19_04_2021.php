...
class Mdl_stock extends CI_Model {
	...
	public function getRentIdleStockByItem($warehouse_id,$itemid)
	{
        ...
        // PURCHASE STOCK STARTS HERE.....
		$query=$this->db->select('sum(qty) as qty')
		->from('purchase_stock_item')
		->where(array('item_id' =>$itemid ,'warehouse_id'=>$warehouse_id ))
		->get();
		$totalpurchase = $query->row();
        // PURCHASE STOCK ENDS HERE.....
...
//	{
//		$query=$this->db->query("select sum(qty) as qty from tbl_inward_rent_item where item_id=".$itemid);
//		$totalinward=$query->row();
//
//		$query=$this->db->query("select sum(qty) as qty from tbl_rent_item_stock_transfer where to_item_id=".$itemid);
//		$totalinwardstock=$query->row();
//
//		$query=$this->db->query("select sum(qty) as qty from tbl_rent_item_stock_transfer where  from_item_id=".$itemid);
//		$totaloutwardstock=$query->row();
//
//		$query=$this->db->query("select sum(qty) as qty from tbl_asign_rent_item where item_id=".$itemid);
//		$totalasignstock=$query->row();
//
//		$query=$this->db->query("select sum(qty) as qty from tbl_consignment_rent_stock_item where  item_id=".$itemid);
//		$totaloutward=$query->row();
//
//		$query=$this->db->query("select sum(qty) as qty from tbl_asset_lost where  item_id=".$itemid);
//		$totaldamage=$query->row();
//
//		$query=$this->db->select("warehouse_opening_stock")
//		->from('tbl_stock_rent_item')
//		->where('item_id',$itemid)
//		->get();
//      $opstock = $query->row();
//
//        $query = $this->db->select("warehouse_id,item_id")
//            ->from('purchase_stock_item')
//            //->join('tbl_item_barcodes','tbl_item_barcodes.item_id=purchase_stock_item.item_id')
//            ->where('item_id',$itemid)
//           //->where('barcode_no<>','')
//            ->where('warehouse_id',$warehouse_id)
//            ->get();
//        echo $this->db->last_query();
//        echo $purstock = $query->num_row();
//        $purStock = $purstock->qty;
//
//		$inwardarray=array();
//		$ctr=0;
//		$x=0;
//		$y=0;
////		$x=$x+$totalinward->qty;
////		$x=$x+$totalinwardstock->qty;
////		$x=$x+$totalasignstock->qty;
////		$x=$x-$totaloutwardstock->qty;
////		$x=$x-$totaloutward->qty;
////		$x=$x-$totaldamage->qty;
////		$x=$x+$opstock->warehouse_opening_stock;
//		$x=$x+$purStock;
//
////		$query=$this->db->query("select sum(qty) as qty from tbl_inward_rent_item where  item_id=".$itemid);
////		$totalinward=$query->row();
...
//	}
    ...
    public function getRentStockByItemInTable($itemid,$warehouse_id="0")
    {
        ...
        // ........PURCHASE STOCK STARTS HERE........ //
        $PurStock=0;
       //echo $sqlPur="SELECT `warehouse_id`, `purchase_stock_item`.`item_id`, `qty`, `tbl_item_barcodes`.`barcode_no`
        //         FROM `purchase_stock_item`
        //         JOIN `tbl_item_barcodes` ON `tbl_item_barcodes`.`item_id`=`purchase_stock_item`.`item_id`
        //         WHERE `purchase_stock_item`.`item_id` = $itemid AND `warehouse_id` = $warehouse_id AND `tbl_item_barcodes`.`barcode_no`!=''";
      
         //$sqlPur="SELECT `warehouse_id`, `purchase_stock_item`.`item_id`, `qty`, `tbl_item_barcodes`.`barcode_no`
         //        FROM `purchase_stock_item`
         ////        JOIN `tbl_item_barcodes` ON `tbl_item_barcodes`.`item_id`=`purchase_stock_item`.`item_id`
         //        WHERE `purchase_stock_item`.`item_id` = $itemid";
        //$query=$this->db->query($sqlPur);
        //$PurStock=$query->num_rows();
        // .........PURCHASE STOCK ENDS HERE........ //
        //echo 66765765; exit();
        ...
        $query=$this->db->select("opening_stock,warehouse_id")
            ...
            ->get();
        $opstock=$query->row();
        $opening_stock = $opstock->opening_stock;
        // ........OPENING STOCK ENDS HERE........ //
        ...
        $x = $x+$opstock->warehouse_opening_stock;
        $opstock->warehouse_opening_stock;
        $stock=($totalin+$PurStock+$opening_stock)-$totalout;
        //return $x+$y;
        return $stock;
    }


    public function getRentStockByItemInWarehouse($itemid,$warehouse_id="0")
	{
		$query=$this->db->query("select sum(qty) as qty from tbl_inward_rent_item where item_id=".$itemid." and warehouse_id=".$warehouse_id);
		$totalinward=$query->row();

		$query=$this->db->query("select sum(qty) as qty from tbl_rent_item_stock_transfer where to_item_id=".$itemid." and consignment_id='0' and to_warehouse_id=".$warehouse_id);
		$totalinwardstock=$query->row();
		/*Outward stock*/
		$query=$this->db->query("select sum(qty) as qty from tbl_rent_item_stock_transfer where  from_item_id=".$itemid." and from_warehouse_id=".$warehouse_id);
		$totaloutwardstock=$query->row();

		$query=$this->db->query("select sum(qty) as qty from tbl_asign_rent_item where item_id=".$itemid." and to_warehouse_id=".$warehouse_id);
		$totalasignstock=$query->row();

		$query=$this->db->query("select sum(qty) as qty from tbl_consignment_rent_stock_item where  item_id=".$itemid." and warehouse_id=".$warehouse_id);
		$totaloutward=$query->row();
		
		$query=$this->db->select("sum(warehouse_opening_stock) as warehouse_opening_stock")
		->from('tbl_stock_rent_item')
		->where(array('item_id' =>$itemid ,'warehouse_id'=> $warehouse_id))
		->get();
		$opstock=$query->row();

		$query=$this->db->query("select sum(qty) as qty from tbl_asset_lost where item_id=".$itemid." and warehouse_id=".$warehouse_id);
		$totaldamage=$query->row();
	...
	public function getstockbyconsignee()
	{
		...
		$query=$this->db->query("SELECT inward_date as date,item_id,sum(qty) as qty,'0' as type,'' as consignment_no,'' as consignment_id,'' as vehicle_inward_no,'' as mobile_no, '' as d_c_no FROM `tbl_inward`  LEFT JOIN tbl_inward_details on tbl_inward_details.inward_id=tbl_inward.inward_id where tbl_inward.destiantion_id='".$consignee_id."' and inward_date>='".$date_from."' and inward_date<='".$date_to."' GROUP by inward_date,item_id
union
SELECT consignment_date as date,item_id,sum(qty) as qty,'1' as type,consignment_no,tbl_consignment.consignment_id,vehicle_inward_no,mobile_no,d_c_no FROM `tbl_consignment`  LEFT JOIN tbl_consignment_stock_item on tbl_consignment_stock_item.consignment_id=tbl_consignment.consignment_id
...
ORDER BY `date`,item_id asc, type,vehicle_inward_no desc");
...
ORDER BY `date`,consignment_no,item_id asc, type desc");
	...
	public function getwarehousestock($itemid,$consignee_id)
	{
		...
		//exit;
		$query=$this->db->query("select sum(qty) as qty,to_warehouse_id as warehouse_id from tbl_rent_item_stock_transfer where to_id='".$consignee_id."' and to_item_id=".$itemid." and consignment_id='0' group by to_warehouse_id");
		...
		$query=$this->db->query("select sum(qty) as qty,warehouse_id from tbl_asset_lost where  item_id=".$itemid. " group by warehouse_id");
		$totaldamage=$query->result_array();
		...
		if(sizeof($totaldamage)>0)
		{
			//var_dump($totaldamage);
			foreach ($totaldamage as $row) {
				# code...
				$pos=array_search($row['warehouse_id'], $warehouse_id_array);
				$inwardarray[$pos]=$inwardarray[$pos]-$row['qty'];
			}
	...
	{
		$query=$this->db->query("select sum(qty) as qty from tbl_inward_rent_item where inward_id in(select inward_id from tbl_inward where rent_consignee_id='".$consignee_id."') and consignment_id='0' and item_id=".$itemid);
		$totalinward=$query->row();
	...
	public function getIntrasit($itemid,$consignee_id)
	{
		$query=$this->db->query("select sum(qty) as qty,to_warehouse_id as warehouse_id from tbl_rent_item_stock_transfer where to_id='".$consignee_id."' and to_item_id=".$itemid." and consignment_id<>'0' and consignment_id not in(select consignment_id from tbl_inward) group by to_warehouse_id");
		$totalinward=$query->result_array();
	...
	public function getCoWiseStock()
	{
		...
		foreach ($record as $row) {
			...
			$temp['intransit']=$this->getIntrasit($row['item_id'],$consignee_id);
			array_push($response['item'], $temp);
		}
...
ORDER BY `date`,type,consignment_no,item_id asc");
	...
	public function getItemWiseReport()
	{
		$master_item_id=$_POST['master_item_id'];
		$query=$this->db->select('item_id,tbl_stock_rent_item.consignee_id,consignee_name')
				->from('tbl_stock_rent_item')
				->join('tbl_consignee','tbl_consignee.consignee_id=tbl_stock_rent_item.consignee_id')
		...
		foreach ($data as $row) {
			$temp=[];
			$temp['consignee_name']=$row['consignee_name'];
			$temp['warehouse_stock']=$this->getwarehousestock($row['item_id'],$row['consignee_id']);
			$temp['current_co_stock']=$this->getcostock($row['item_id'],$row['consignee_id']);
			array_push($response, $temp);
		}
		$temp=[];
		$temp['consignee_name']='Idel Item';
		$temp['warehouse_stock']=$this->getIdleItem($master_item_id);
		$temp['current_co_stock']='0';
		array_push($response, $temp);
		return $response;
	}


	public function getIdleItem($master_item_id)
	{
		$query=$this->db->select('sum(qty) as qty,warehouse_id')
						->from('purchase_stock_item')
						->where('item_id',$master_item_id)
						->group_by('warehouse_id')
						->get();
		$totalpurchase=$query->result_array();

		$query=$this->db->select('sum(qty) as qty,from_warehouse_id')
						->from('tbl_asign_rent_item')
						->where('master_item_id',$master_item_id)
						->group_by('from_warehouse_id')
						->get();
		$totalassign=$query->result_array();
		$query=$this->db->select('warehouse_id,warehouse_name')
		->from('tbl_warehouse')
		->get();
		...
		foreach ($totalpurchase as $row) {
			# code...
			$pos=array_search($row['warehouse_id'], $warehouse_id_array);
			$stock[$pos]=$stock[$pos]+$row['qty'];

		}

		foreach ($totalassign as $row) {
			# code...
			$pos=array_search($row['from_warehouse_id'], $warehouse_id_array);
			$stock[$pos]=$stock[$pos]-$row['qty'];
...
// 	public function getWarehouseWiseReport()
// 	{
// 		$warehouse_id=$_POST['warehouse_id'];
// 		$query=$this->db->select('master_item_id,item_name,item_code')
// 		->from('tbl_rent_item_master')
// 		->where('is_deleted','0')
// 		->get();
...
// 			$arr=[];
// 			$arr['item_name']=$row['item_name'];
// 			$arr['item_code']=$row['item_code'];
// 			$arr['qty']=$this->getRentIdleStockByItem($warehouse_id,$row['master_item_id']);
// 			array_push($response, $arr);
// 		}
// 		return $response;
// 	}

    public function getWarehouseWiseReport($warehouse_id)
    {
    		$warehouse_id = $_POST['warehouse_id'];
            //if(is_numeric($warehouse_id)){
                 $query=$this->db->select('master_item_id,item_name,item_code,tbl_warehouse.warehouse_id,warehouse_name')
                     ->from('tbl_rent_item_master')
                     ->join('tbl_warehouse',"`tbl_warehouse`.`warehouse_id` = $warehouse_id")
                     ->where('tbl_rent_item_master.is_deleted','0')
                     ->get();
                 // echo $this->db->last_query();
                 // echo "</br>"; exit();
                 $result = $query->result_array();
                 ...
                 foreach ($result as $row)
                 {
                     ...
                     $arr['item_name']=$row['item_name'];
                     $arr['item_code']=$row['item_code'];
                     $arr['warehouse_name']=$row['warehouse_name'];
                     $arr['qty']=$this->getRentIdleStockByItem($warehouse_id,$row['master_item_id']);
                     $arr['item_id']=$row['master_item_id'];
                     array_push($response, $arr);
                 }
            // }
...
//             {
//                 echo "<pre>";
//                // WAREHOUSE RESULT STARTS HERE..
//                 $res = $this->db->select('warehouse_name,warehouse_id')
//                     ->from('tbl_warehouse')
//                     ->get()->result();
//                 // WAREHOUSE RESULT STARTS HERE..
//                 foreach($res as $r):
...
//             }
    ...
    {
        $query=$this->db->select('`master_item_id`,`item_name`')
            ->from('tbl_rent_item_master')
    ...
    public function getbarcodesbyitemIDWarehouseID($warehouse_id,$item_id)
    {
        $query = $this->db->select('distinct(`barcode_id`), `barcode_no`, `item_name`, `tbl_item_barcodes`.`item_id`,`tbl_item_barcodes`.`booking_id`,`purchase_stock_item`.`warehouse_id`')
            ->from('tbl_item_barcodes')
            ->join('tbl_rent_item_master','tbl_rent_item_master.master_item_id=tbl_item_barcodes.item_id','left')
            ->join('purchase_stock_item','purchase_stock_item.booking_id=tbl_item_barcodes.booking_id','left')
            ->where('`tbl_item_barcodes`.`item_id`',$item_id)
            ->where('`tbl_item_barcodes`.`is_active`',1)
            ->where('`purchase_stock_item`.`warehouse_id`',$warehouse_id)
            ->get();
        //echo $this->db->last_query(); exit;
