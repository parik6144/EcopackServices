...
//       else $consignment_type=$_POST['consignment_type'];
        ...
        if(strtotime($date1)< strtotime($consignment_date) && $is_dc==0)
        {
            $qry=$this->db->select('max(consignment_no) as consignment_no')
                ...
                ->where('consignment_date >',"2022-03-31")
                ->where('consignment_id >',"13457")
                ->get();
...
//exit();
        ...
        { 
          // echo 897897; exit();
            ...
            if(is_null($row->dc_no) || $row->dc_no<=0){ $dc_no="1"; } else { $dc_no=$row->dc_no+1; }
       ...
       // echo sizeof($transportdata); exit();
        ...
        if(sizeof($rentdata)>0)
           ...
           $is_dc = $_POST['is_dc'];  
           if($is_dc==0){ redirect('consignment/index'); }
            if($is_dc==1){ redirect('Deliverychallan/index'); }
        return true;
    }
	...
	public function getconsignmentdc($start = "", $length = "", $searchstr = "", $column, $type, $consignee)
	{
		...
		if ($start == '' && $length == '' && $searchstr == '') {
			$query = $this->db->select('`consignment_id`, `consignment_no`,`tbl_consignment`.`consignee_id`, `tbl_consignment`.`invoice_id`, `dc_no`,`consignment_date`, tbl_consignment.consignor_id, tbl_consignment.consignee_id, `source_id`, `destination_id`, tbl_consignment.vehicle_id, `total_amount`, `way_bill`, `description`,consignor_name,consignee_name,p.place_name as source,pa.place_name as destination,vehicle_no,`tbl_consignment`.`consignment_warehouse_id`,tbl_warehouse.warehouse_city')
				->from('tbl_consignment')
				->join('tbl_consignor', 'tbl_consignor.consignor_id = tbl_consignment.consignor_id', 'left')
				->join('tbl_consignee', 'tbl_consignee.consignee_id = tbl_consignment.consignee_id', 'left')
				->join('tbl_vehicle_inward', 'tbl_vehicle_inward.vehicle_inward_id = tbl_consignment.vehicle_id', 'left')
			...
			if (!empty($consignee)) {
				$this->db->where('tbl_consignment.consignee_id', $consignee);
			}
			...
			$this->db->select('consignment_id,consignment_no,`tbl_consignment`.`consignee_id`,`tbl_consignment`.`invoice_id`, `dc_no`,consignment_date, consignor_name, consignee_name, bill_type, vehicle_inward_no,tbl_consignment.driver_name,consignment_status,`tbl_consignment`.`consignment_warehouse_id`,tbl_warehouse.warehouse_city');
			...
			$this->db->order_by('consignment_date', 'desc')
				->order_by('dc_no', 'desc');
    ...
    public function updateconsignment($post)
    {
        ...
        if(empty($_POST['way_bill_no'])) $way_bill_no="";
        else $way_bill_no=$_POST['way_bill_no'];

        if(empty($_POST['driver_name'])) $driver_name=""; else $driver_name=$_POST['driver_name'];

        if(empty($_POST['advance'])) $advance=""; else $advance = $_POST['advance'];

        if(empty($_POST['due'])) $due=""; else $due = $_POST['due'];
